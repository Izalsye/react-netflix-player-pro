...
export default function PrimeVideoDoc({ lang }: DocProps) {
    const [docLang, setDocLang] = useState<'html' | 'react' | 'django'>('html');

    // 🌐 Dictionary untuk Terjemahan
    const t = {
        title: lang === 'id' ? 'Cara Play Video (Prime Video DRM)' : 'How to Play Video (Prime Video DRM)',
        desc: lang === 'id'
            ? <>Response API dari Prime Video terenkripsi menggunakan <strong>Widevine DRM</strong>. Untuk memutar video, Anda harus menginisialisasi <a href="https://shaka-player-demo.appspot.com/docs/api/shaka.Player.html" target="_blank" className="text-[var(--primary)] hover:underline font-semibold">Shaka Player</a> dengan mengarahkan <code className="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-rose-500 text-xs font-mono">dashUrl</code> ke player, dan <code className="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-rose-500 text-xs font-mono">drm_license_url</code> ke konfigurasi Widevine server.</>
            : <>The API response from Prime Video is encrypted using <strong>Widevine DRM</strong>. To play the video, you must initialize <a href="https://shaka-player-demo.appspot.com/docs/api/shaka.Player.html" target="_blank" className="text-[var(--primary)] hover:underline font-semibold">Shaka Player</a> by passing the <code className="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-rose-500 text-xs font-mono">dashUrl</code> to the player, and <code className="bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-rose-500 text-xs font-mono">drm_license_url</code> to the Widevine server configuration.</>,
    };
    ...
    return (
        <div className="bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-[#161b22] px-6 py-4">
                <Box size={20} className="text-emerald-500 dark:text-emerald-400" />
...
async function initApp() {
    ...
    const videoUrl = "ISI_DENGAN_dashUrl_DARI_RESPONSE"; 
    const licenseUrl = "ISI_DENGAN_drm_license_url_DARI_RESPONSE";

    // 3. ${lang === 'id' ? 'Konfigurasi DRM (Wajib L3 Software Decryption)' : 'DRM Configuration (L3 Software Decryption Required)'}
    player.configure({
        drm: {
...
export default function PrimePlayer({ responseData }) {
    ...
    useEffect(() => {
        ...
        const videoUrl = responseData.dashUrl;
        const licenseUrl = responseData.licenseServers.drm_license_url;
...
<script>
    // Django / Jinja2:
    const videoUrl = "{{ dash_url|escapejs }}";
    const licenseUrl = "{{ license_url|escapejs }}";

    // PHP Laravel Blade:
    // const videoUrl = "{{ $dashUrl }}";
