    ...
    useEffect(() => {
        ...
        const initPlayer = async () => {
            if (isMp4) {
                ...
                // 🔥 1. CONFIG DEWA (Udah disesuaikan dengan HLS.js versi baru biar TS aman) 🔥
                const hls = new Hls({
                    maxBufferHole: 2, // Toleransi kalau ada gap/bolong waktu 2 detik (Ini masih didukung TS)
                });

                hlsRef.current = hls;

                // 🔥 2. PENANGKAL ERROR (Membungkam kepanikan hls.js) 🔥
                hls.on(Hls.Events.ERROR, (event, data) => {
                    if (data.fatal) {
                        switch (data.type) {
                            case Hls.ErrorTypes.MEDIA_ERROR:
                                console.warn('🎬 [HLS.js] Nemuin glitch WeTV di perbatasan VIP, recovering...');
                                hls.recoverMediaError(); // Jurus pamungkas: Benerin timeline tanpa skip!
                                break;
                            case Hls.ErrorTypes.NETWORK_ERROR:
                                console.warn('🌐 [HLS.js] Network error, coba load ulang...');
                                hls.startLoad();
                                break;
                            default:
                                hls.destroy();
